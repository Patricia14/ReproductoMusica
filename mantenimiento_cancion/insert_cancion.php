<?php
session_start();
?>
<!DOCTYPE html>
<html lang="en">

<head>

    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="description" content="">
    <meta name="author" content="">

    <title>Administración Musica Online</title>

    <!-- Bootstrap Core CSS -->
    <link href="../recursos/css/bootstrap.min.css" rel="stylesheet">

    <!-- Custom CSS -->
    <link href="../recursos/css/sb-admin.css" rel="stylesheet">

    <!-- Morris Charts CSS -->
    <link href="../recursos/css/plugins/morris.css" rel="stylesheet">

    <!-- Custom Fonts -->
    <link href="../recursos/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">

   
</head>

<body>
 <div id="wrapper">

        <!-- Navigation -->

       <?php
       $titulo="";
  $genero="";
  $version="";
  $album="";
  $sello="";
$foto="";
$cancionNombre="";
$msgTitulo= "";
$msgVersion= "";
$msgFoto= "";
$msgCancion="";
$msgCombo="";



 require ("../conexion/management.php");
      $obj = new management();
      $datosGenero=$obj->consulta("SELECT * FROM genero ");
      $datosAlbum=$obj->consulta("SELECT * FROM album ");
      $datosSello=$obj->consulta("SELECT * FROM sello ");
         require('../menu/menuLateralAdmin.php');

if($_POST){

    include ("../mantenimiento_cancion/insercion_cancion.php");
}
       
       ?>
       <!-- End Navigation-->
        <div id="page-wrapper">
            <div class="container-fluid">
                <!-- Page Heading -->
              
                    <div class="col-lg-12">
                        <h1 class="page-header">
                          Datos del Canción
                        </h1>
                         
                <form class="form-horizontal" method="POST" action="../mantenimiento_cancion/insert_cancion.php" id="formCliente" enctype="multipart/form-data">
               
                  <div class="form-group">
                    <label for="titulo" class="control-label col-xs-5">Titulo:</label>
                    <div class="col-xs-5">
                        <input id="txttitulo" name="txttitulo" type="text" class="form-control" value="<?php echo $titulo;?>">
                        <div class="msg"><?php echo $msgTitulo; ?></div>
                    </div>
                    
                  </div>
                  <div class="form-group">
                    <label for="genero" class="control-label col-xs-5">Genero:</label>
                    <div class="col-xs-5">
<select  name="txtgenero" id="txtgenero">
                         <?php foreach ($datosGenero as $fila) {
if (!empty($genero)) {
                           echo "<option value='".$genero."'>Valor guardado</option>";
                         }
                               echo "<option value='".$fila["id_Genero"]."'>".$fila["nombre_Genero"]."</option>"; 

                          }?>
                        </select>
<div class="msg"><?php echo $msgCombo; ?></div>
                     <!--   <input id="txtgenero" name="txtgenero" type="text" class="form-control" >-->
                    </div>
                  </div>
                   
                  <div class="form-group">
                    <label for="version" class="control-label col-xs-5">Version:</label>
                    <div class="col-xs-5">
                        <input id="txtversion" name="txtversion" type="text" class="form-control" value="<?php echo $version;?>">
                        <div class="msg"><?php echo $msgVersion; ?></div>
                    </div>
                  </div>
                  <div class="form-group">
                    <label for="album" class="control-label col-xs-5">Album:</label>
                    <div class="col-xs-5">
            
<select  name="txtalbum" id="txtalbum">
                         <?php 
                         if (!empty($album)) {
                           echo "<option value='".$album."'>Valor guardado</option>";
                         }
                           
                         foreach ($datosAlbum as $fila) {

                               echo "<option value='".$fila["id_Album"]."'>".$fila["nombre_Album"]."</option>"; 

                          }?>
                        </select>
<div class="msg"><?php echo $msgCombo; ?></div>
                        <!--<input id="txtalbum" name="txtalbum" type="text" class="form-control" >-->
                    </div>
                  </div>
                  
                  <div class="form-group">
                    <label for="sello" class="control-label col-xs-5">Sello:</label>
                    <div class="col-xs-5">
<select  name="txtsello" id="txtsello">
                         <?php foreach ($datosSello as $fila) {
if (!empty($sello)) {
                           echo "<option value='".$sello."'>Valor guardado</option>";
                         }
                               echo "<option value='".$fila["id_Sello"]."'>".$fila["nombre_Sello"]."</option>"; 

                          }?>
                          
                        </select>
<div class="msg"><?php echo $msgCombo; ?></div>

                       <!-- <input id="txtsello" name="txtsello" type="text" class="form-control" >-->
                    </div>
                  </div>
                 
                  <div class="form-group">
                    <label for="cancion" class="control-label col-xs-5">Ruta de canción:</label>
                    <div class="col-xs-5">
         
<input id="txtcancion" name="txtcancion" type="file" class="form-control" >
 <div class="msg"><?php echo $msgCancion; ?></div>

 </div>
                  </div>
                  <!--<div class="form-group">
                    <label for="cancion" class="control-label col-xs-5">Ruta de canción:</label>
                    <div class="col-xs-5">
                        <input id="txtcancion" name="txtcancion" type="text" class="form-control" >
                    </div>
                  </div>-->

                   <div class="form-group">
                    <label for="imagen" class="control-label col-xs-5">Imagen de canción:</label>
                    <div class="col-xs-5">
                        <input id="txtimagen" name="txtimagen" type="file" class="form-control" >
                        <div class="msg"><?php echo $msgFoto; ?></div>
                    </div>
                  </div>
                 <div class="modal-footer">  
                <a href="../navegacion_admin/adminCancion.php" class="btn btn-danger" role="button">Regresar</a>
                <input type="submit" name="envio" class="btn btn-success" value="Guardar">
                
              </div>
                           <input type="hidden" name="funcion" value="insertar"/>
     </form>
             
       
        </div><!-- /.modal -->
                    </div>
                </div>
                <!-- /.row -->
              </div>
            <!-- /.container-fluid -->

        </div>
        <!-- /#page-wrapper -->


    </div>
    <!-- /#wrapper -->

    <!-- jQuery -->
    <script src="../recursos/js/jquery.js"></script>

    <!-- Bootstrap Core JavaScript -->
    <script src="../recursos/js/bootstrap.min.js"></script>

    <!-- Morris Charts JavaScript -->
    <script src="../recursos/js/plugins/morris/raphael.min.js"></script>
    <script src="../recursos/js/plugins/morris/morris.min.js"></script>
    <script src="../recursos/js/plugins/morris/morris-data.js"></script>

    
<?php
    require('../menu/footer.php');
?>
</body>


</html>
